...
const ADMIN_EMAILS = ["admin@turismo.com", "turismo@admin.com"]
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const unsub = onAuthStateChanged(auth, async (fbUser) => {
      if (fbUser) {
        ...
        setUserData({ uid: fbUser.uid, email: fbUser.email || "", name: nameFromDoc, role })
  ...
  const signIn = async (email: string, password: string) => {
    if (USE_MOCK) { setUserData({ uid: "mock-uid", email, name: "", role: "viewer" }); return }
    ...
    const nameFromDoc = userDoc.exists() ? userDoc.data().name || "" : ""
    setUserData({ uid: cred.user.uid, email: cred.user.email || "", name: nameFromDoc, role })
  }
  ...
  const signUp = async (name: string, email: string, password: string) => {
    ...
    setUserData({ uid: cred.user.uid, email: cred.user.email || "", name, role: "viewer" })
  }

  const signInWithGoogle = async () => {
    if (USE_MOCK) { setUserData({ uid: "mock-uid", email: "viajero@mock.com", name: "", role: "viewer" }); return }
  ...
  const signOut = async () => {
    if (USE_MOCK) { setUserData(null); return }
  ...
  return (
    <AuthContext.Provider value={{
      user: resolvedUser, role: resolvedRole, isAdmin: resolvedIsAdmin,
      loading: USE_MOCK ? false : loading, signIn, signUp, signInWithGoogle,
      signOut, setMockRole, checkEmailExists, resetPassword,
    }}>
      {children}
