  ...
  persist(
    (set, get) => ({
      ...
      setScenario: (scenario) =>
        set({ scenario, scenarioConfig: SCENARIOS[scenario] }),
      ...
      initFirestoreSync: () => {
        ...
        const unsub = onSnapshot(ref, (snap: any) => {
          if (snap.exists()) {
            ...
            if (data.config) {
              set((state: any) => ({ params: { ...state.params, ...data.config } }))
            }
          }
        }, () => {})
        return unsub
      ...
      saveToFirestore: async () => {
        ...
        if (!isFirestoreAvailable) return
        try {
          await setDoc(doc(db, PARAMS_DOC), { config: get().params, updatedAt: serverTimestamp() }, { merge: true })
        } catch { /* silent fail */ }
      },
    }),
    { name: "turismo-params", partialize: (state) => ({ params: state.params }) }
  )
)
