...
export function usePromotions() {
  return useQuery({
    ...
    queryFn: async (): Promise<Promotion[]> => {
      ...
      return snap.docs.map((d: any) => ({ id: d.id, ...d.data() } as Promotion))
    },
  })
}
...
export function useSubscribers() {
  return useQuery({
    ...
    queryFn: async (): Promise<Subscriber[]> => {
      ...
      return snap.docs.map((d: any) => ({ id: d.id, ...d.data() } as Subscriber))
    },
  })
}
...
export function useSavePromotion() {
  ...
  return useMutation({
    mutationFn: async (data: Omit<Promotion, "id" | "createdAt"> & { id?: string; createdAt?: string }) => {
      ...
      await setDoc(doc(db, "promotions", id), { ...data, id, createdAt: data.createdAt || new Date().toISOString() })
